...
- **Tools（工具）**: AI 可调用的函数，用于执行操作或查询信息
- **Prompts（提示词）**: 预定义的提示模板
- **Resources（资源）**: 可访问的资源列表
- **Tasks（任务）**: 异步任务管理系统
...
| `EnableSceneTools` | bool | true | 启用场景工具 |
| `EnableConsoleTools` | bool | true | 启用控制台工具 |
| `EnableTimeTools` | bool | true | 启用时间工具 |
...
| `get_gameobject_children` | 获取子物体列表 |
| `set_gameobject_active` | 设置激活状态 |
| `log_message` | 输出日志到控制台 |
...
| `EnableSceneTools` | bool | true | 启用场景操作内置工具 |
| `EnableConsoleTools` | bool | true | 启用控制台内置工具 |
| `EnableTimeTools` | bool | true | 启用时间控制内置工具 |
